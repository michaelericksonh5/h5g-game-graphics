...
const SLOT_CONTEXT = [
  ...
  "4) slot-backgrounds-and-frames (painted scene background + reel frame/bezel/chassis, material-shaded, baked) →",
  "5) procedural-symbol-design (draw symbols) →",
  "6) slot-symbol-animation-states (symbol state animations) →",
  "7) slot-bonus-features (bonus/free-spins features) →",
  "8) slot-hud-and-ui (UI shell; repo placement/P4 → webgamedev-structure) →",
  "9) particle-systems-and-juice (game feel) →",
  "10) tonejs-game-audio (audio FOUNDATION/unlock/bus) → procedural-sfx-design (event SFX) + adaptive-game-music (music/stems) →",
  "11) game-qa-and-testing (QA/sign-off LAST: 60fps, visual rubric, device matrix — do not report done without it).",
